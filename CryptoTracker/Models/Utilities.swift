...
func binarySearch(crypto: [Cryptocurrency], start: Int, end: Int, key: String)->Int {
    ...
    while left <= right  && found == -1 {
        ...
        if crypto[mid].icon.symbol.lowercased() == keyL {
            found = mid
        } else if crypto[mid].icon.symbol.lowercased() < keyL {
            left = mid + 1
        } else if crypto[mid].icon.symbol.lowercased() > keyL {
            right = mid - 1
        }
...
func getJsonRequest(url: String, parameters: [String: Any], function: @escaping (JSON) -> Void) {
